# 궁금하면 구현해보는 개발자

> 게임의 가장 큰 목표인 재미를 목표로 나아가고 싶습니다.

안녕하세요. 저는 **Unity**와 **C\#**을 활용하여 게임을 개발하고 있는 신현빈입니다.

---

## 연락처 및 링크

| 항목 | 정보 |
| :--- | :--- |
| **GitHub** | [https://github.com/nanasin120](https://github.com/nanasin120) |
| **Email** | `tlsgusqls120@dankook.ac.kr` |
| **배포 프로젝트** | [살인사건 - 펜션 (GamePing)](https://www.game-ping.kr/games/murder-at-mension) |

---

## 핵심 기술 스택

| 카테고리 | 기술 스택 | 주요 경험 |
| :--- | :--- | :--- |
| **게임 개발 엔진** | **Unity** | 2D 게임 프로젝트 경험 (Top-Down RPG 프레임워크 개발 중) |
| **주요 언어** | **C\#** | Unity 핵심 로직 및 매니저 시스템 개발에 사용 |
| **프로토타이핑** | **HTML, JavaScript, CSS** | 알고리즘 및 아이디어를 빠르고 직관적으로 시각화 (A\* 알고리즘, Raycaster 구현) |

---

## 주요 프로젝트

### 1. A\* Pathfinding 알고리즘 구현 (웹 시각화)

> 게임 AI가 길찾기에 이용하는 **A\* 알고리즘**을 직접 구현한 프로젝트

* **만든 이유**: 2d 게임에서 적이 플레이어를 추적하는데 사용되는 알고리즘인 A* 알고리즘을 확실히 이해하기 위해 구현.
* **획득 경험**: 알고리즘에 대한 이해와 **JavaScript**를 활용한 **실시간 경로 탐색 시각화** 구현 경험.
* **기술 스택**: `HTML`, `JavaScript`, `CSS`
* **GitHub 링크**: [https://github.com/nanasin120/a-star-algorithm-in-web](https://github.com/nanasin120/a-star-algorithm-in-web)

### 2. raycaster 구현 (웹 시각화)

> 고전 3D 게임의 렌더링 방식인 **Raycasting**의 원리를 웹에서 구현한 프로젝트

* **만든 이유**: 과거 3D 게임이 어떻게 렌더링되었는지에 대한 **기술적 호기심**을 해소하기 위해 구현.
* **획득 경험**: **광선 추적(Raycasting)** 개념의 이해 및 수학적 지식.
* **기술 스택**: `HTML`, `JavaScript`, `CSS`
* **GitHub 링크**: [https://github.com/nanasin120/raycaster](https://github.com/nanasin120/raycaster)

### 3. 살인사건 - 펜션

> **GamePing 10분 컨테스트**에 제출한 간단한 추리 게임

* **주요 성과**: **10분**이라는 제약된 시간 내에 아이디어를 구체화하고, 플레이 가능한 형태로 구현 및 제출.
* **획득 경험**: 짧은 기간 동안 **기획 의도**에 맞는 **핵심 게임 플레이**를 빠르게 구현하고 배포하는 능력.
* **기술 스택**: `C#`, `Unity`
* **배포 링크**: [GamePing에서 플레이하기](https://www.game-ping.kr/games/murder-at-mension)
* **개발 이후**: 게임 개발에 있어 기본 프레임워크의 필요성을 인식

### 4. Unity RPG Manager (개발 중)

> 2D Top-Down 게임 개발의 효율성을 높이기 위한 재사용 가능한 **기본 프레임워크**

* **만든 이유**: 10분 컨테스트용 게임을 만들면서 2D Top-Down 게임을 만드는 **기본 틀(Manager System)**이 있으면 새로운 게임을 만들 때 편할 것이라는 생각에서 시작.
* **주요 목표**: 기본 프레임워크를 개발함으로서 앞으로의 게임 제작을 보다 더 편하게 하기 위함
* **기술 스택**: `C#`, `Unity`
* **GitHub 링크**: [https://github.com/nanasin120/unity\_rpg\_manager](https://github.com/nanasin120/unity_rpg_manager)

---
